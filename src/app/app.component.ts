...
export class AppComponent implements OnInit {
  ...
  polygon() {
    ...
    const createRectangle = () => {
      ...
      rect?.addEventListener('mousedown', (e) => {
        ...
        rectStartPoint = [
          parseFloat(this.currentRectangle.getAttribute('x') || '0'),
          parseFloat(this.currentRectangle.getAttribute('y') || '0')
        ];
    ...
    const createPolygon = (id: any): SVGElement | null => {
      if (this.currentRectangle) {
        ...
        poly.setAttribute('fill', 'transparent'); // Optional fill color for the polygon
        poly.setAttribute('stroke', 'black'); // Border color for the polygon
        poly.setAttribute('stroke-width', '2');
        poly.setAttribute('id', `${id}polygon`);
  ...
  ellipse() {
    ...
    const updateEllipse = () => {
      if (this.currentRectangle && this.currentSVGElement) {
        ...
        const centerX = rectX + width / 2;
        const centerY = rectY + height / 2;
        const rx = (width / 2) - 8; // x-radius
        const ry = (height / 2) - 8; // y-radius
  ...
  doubleArrowLine() {
    ...
    const createLine = () => {
      ...
      let line = document.createElementNS(SVG_NAMESPACE, 'line');
      line.setAttribute('stroke', 'black'); // Line color
      line.setAttribute('stroke-width', '2');// Rounded joins to smoothen the curve
    ...
    const createArrowhead = () => {
      ...
      arrowhead3.setAttribute('stroke-width', '2');
      ...
      arrowhead4.setAttribute('stroke-width', '2');
